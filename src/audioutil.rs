...
//! (microphone capture) and the bundled whisper transcriber. Compiled only when
//! one of those features is on.
//!
//! The recorder writes the WAV at the device's native rate (so the muxed video
//! keeps decent audio and capture stays a cheap streaming write); resampling to
//! whisper's 16 kHz happens only in the `whisper` transcriber, so those helpers
//! are gated to that feature to stay dead-code-free in a `record`-only build.
...
pub(crate) fn resample_linear(input: &[f32], from_hz: u32, to_hz: u32) -> Vec<f32> {
    ...
    // Number of output samples = len * to/from.
...
/// Downmix to mono and resample to 16 kHz in one step (whisper input prep).
#[cfg(feature = "whisper")]
...
mod tests {
    ...
    fn resample_scales_length() {
        ...
        assert_eq!(resample_linear(&input, 0, 16_000).len(), input.len());
    }
